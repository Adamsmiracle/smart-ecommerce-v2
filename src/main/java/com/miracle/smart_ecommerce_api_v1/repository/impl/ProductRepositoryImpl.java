...
import com.miracle.smart_ecommerce_api_v1.repository.mapper.ProductMapper;
...
public class ProductRepositoryImpl implements ProductRepository {
    ...
    public void updateStock(UUID productId, int quantity) {
        ...
        int rowsAffected = jdbcTemplate.update(sql, quantity, Timestamp.valueOf(LocalDateTime.now()), productId);
        if (rowsAffected == 0) {
    ...
    public void setActiveStatus(UUID id, boolean isActive) {
        ...
        int rowsAffected = jdbcTemplate.update(sql, isActive, Timestamp.valueOf(LocalDateTime.now()), id);
        if (rowsAffected == 0) {
            throw ResourceNotFoundException.forResource("Product", id);
        }
    }
